...
In the multi-view modelling approach, a set of models is developed for the different aspects of a system.
...
It is sufficient to interprete \autoref{fig:fsm_pn_alignment} as defining the existence of a set of pairs of transitions $SynchCom \subseteq \delta \times F$.
A different formalisation suitable for more complex model alignments is the comprehensive system construction \cite{stunkelMultipleModelSynchronization2020}.

% Transformation of FSM's, petri-nets and maybe the pi-calculus.
For the reasons stated earlier we will choose graph grammars as a semantic domain but one could also choose a different domain or switch in the future.
Our approach is to transform each model into a graph grammar and combine the resulting grammars to realise the defined coordination between the models.
...
Each transition is transformed to a production rule which removes tokens from the incoming edges of the transition and adds tokens to the outgoing places of the transition according to the defined weights.
