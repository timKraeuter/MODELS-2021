...
However, there is no general approach to check joint behavioral consistency on a collection of models, sometimes also called semantic consistency.
...
We will use graph grammars as a behavioral formalism for the model execution (see \autoref{sec:currentStatus}) because of the following reasons.
...
We will use the the example models depicted in \autoref{fig:running_example} to explain the current status of our work by following the proposed methodology.
\autoref{fig:running_example} depicts three state machine $SM^1$-$SM^3$ and a petri net $N^1$ (ignore the cyan connections for now).
As a convention petri net edges have the weight one if not explicitly stated otherwise.
The state machines $SM^1$ and $SM^2$ represent two resources which can be acquired and released by the processes defined by the models $SM^3$ and $N_1$.
$SM^3$ acquires both resources does some work and then releases them again, while the three processes modelled in the petri net $N^1$ only need one of the two resources to do their work.
...
What exactly the processes are working on is kept abstract but we often face situations in which multiple processes are accessing the same resources.
A reasonable requirement for the overall system resulting from running the models would be that all processes always finish their execution and that each resource is accessed by at most one process at a time.
...
The first constraint can be seen as a liveness property while the second and third constraint describe safety properties.
...
A concrete syntax for the metamodel is also given, where edge weights should be written above the arrow depicting the edge.
...
I started to work on integrating the $\pi$-calculus in the semantics domain of graph grammars based on \cite{gadducciGraphRewritingPcalculus2007} but I will not discuss it here.
