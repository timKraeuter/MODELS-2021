...
This is also called \textit{metamodel alignment} and a key activity in the multi-model consistency management process \cite{stunkelMultipleModelSynchronization2020}.
For this approach to work metamodels for the used models have to be present.
However, developing a metamodel if not already present is a one-time expense.
...
After aligning the metamodels one can define inter-model relations between the behavioral models.
For example, the synchronisation of a transition in a state machine with a transition in a petri net.

% 2. Transformation to a semantic domain (proposes graph grammars)
The second step is to transform the models into a suitable semantic domain respecting their inter-model relations.
My current approach (see section \ref{sec:currentStatus}) is to translate each model individually and then combine them according to the defined inter-model relations.

We will use graph grammars as a behavioral formalism for the models because of several reasons.
...
The last step is to check and define behavioral properties.
In this step we assume a state space, i.e. a kripke structure to be present to check behavioral constraints on.
By which semantic domain this state space is generated should not influence the specification of behavioral constraints, such that the choice of a semantic domain does not matter for consistency checking.
We assume there is a set of distinct atomic propositions for each model, such that we can use their union to specify constraints for the overall system.
Besides checking general properties such as deadlocks, I want user definable properties in a temporal logic.
I hope to reuse existing model checkers for evaluating temporal logic formulas on the given kripke structure.
Especially safety properties, i.e. that specific combined states are not reachable seem useful.
